...
# Trimming dataset sizes to ensure stability and prevent Exit Code 137 (OOM)
MAX_TRAIN = 500
MAX_VAL = 100

def save_json(path, data):
    """Utility to save formatted data to a JSON file."""
...
def run_preprocessing():
    ...
    # Select subsets using min() to avoid index out of bounds errors
    ...
    def format_qa(dataset):
        # Format: [{"context": "str", "question": "str", "answers": {"text": [], "answer_start": []}}, ...]
        formatted = []
        for row in dataset:
            formatted.append({
                "context": row["context"],
    ...
    print(f"Data processing complete! All 6 files saved to: {processed_dir}")
